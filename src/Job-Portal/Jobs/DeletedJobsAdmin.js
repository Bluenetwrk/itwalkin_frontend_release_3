...
function Home() {
  ...
  const npage = Math.ceil(totalCount / recordsPerPage) // last page
    ...
    async function gettotalcount() {
      ...
      await axios.get("/jobpost/getTotalCountDeletedJobseeker", { headers })
        .then((res) => {
    ...
    async function getjobs() {
      ...
      await axios.get(`/jobpost/getLimitDeletedJobseeker/${recordsPerPage}`, { params: { currentPage }, headers })
    ...
    function handleRecordchange(e){  
      ...
      setCurrentPage(1)
        ...
        async function getTagId() {
          ...
          await axios.get(`/jobpost/DeletedJobseekerTagsIds/${uniqueList}`, {
  ...
  return(
    ...
    async function changeTags(key){
      ...
      await axios.get(`/jobpost/getTagsDeletedJobseekers/${Active}`)
        .then((res) => {
          let result = (res.data)
                    ...
                    return (
                      ...
                      <ul className={styles.ul} key={i}>
                        ...
                        <li className={`${styles.li} ${styles.Jtitle}`} onClick={() => 
                          navigate(`/BIAddmin@CheckDeletedJob/${items._id}`)} style={{ cursor: "pointer", textDecoration: "underline", color: "blue" }}>{items.jobTitle.charAt(0).toUpperCase()+items.jobTitle.substring(1)}</li>
