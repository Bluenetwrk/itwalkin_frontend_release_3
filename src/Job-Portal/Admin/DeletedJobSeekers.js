...
function ArchivedUser() {
  ...
  const [Candidate, setCandidate] = useState([])
    ...
    async function gettotalcount() {
      ...
      await axios.get("/StudentProfile/getTotalCountDeletedJobSeeker", { headers })
        .then((res) => {
    ...
    async function getAllJobSeekers() {
      ...
      await axios.get(`/StudentProfile/getLimitDeletedJobseeker/${recordsPerPage}`, { params: { currentPage }, headers })
    ...
    function handleRecordchange(e){  
      ...
      const [count, setCount]=useState(1)
        ...
        async function getTagId() {
          ...
          await axios.get(`/StudentProfile/DeletedJobSeekerTagsIds/${uniqueList}`, {
  ...
  return(
    ...
    async function changeTags(key){
      ...
      await axios.get(`/StudentProfile/getTagsDeletedJobSeeker/${Active}`)
        .then((res) => {
          let result = (res.data)
...
<>
    ...
    onClick={()=>{navigate(`/BIAddmin@CheckStudentDeleted/${items.Archived._id}`)}}><Link style={{color:"blue"}}>
     {items.Archived.name}
      </Link></li>
